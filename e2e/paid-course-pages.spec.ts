...
test.describe('Paid / deep course pages (public)', () => {
  ...
  test('paid hosted flagship: access line and launch controls', async ({ page }) => {
    ...
    await expect(page.getByTestId('paid-hosted-interactive-flagship-section')).toBeVisible({ timeout: 20_000 })
    await expect(page.getByTestId('paid-hosted-flagship-access')).toBeVisible()
    await expect(page.getByTestId('paid-interactive-launch-course')).toBeVisible()
    await expect(page.getByTestId('paid-interactive-open-new-tab')).toBeVisible()
  })
