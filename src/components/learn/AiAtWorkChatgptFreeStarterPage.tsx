...
import {
  ...
  AI_AT_WORK_MICROLEARNING_METADATA_ROW,
  AI_AT_WORK_MICROLEARNING_OUTCOMES,
  AI_AT_WORK_MICROLEARNING_SUPPORT_NOTE,
} from '../../data/learning/aiAtWorkMicrolearningPageCopy'
import {
    ...
    >
      <div className="mx-auto w-full max-w-3xl [word-spacing:normal] [letter-spacing:normal]">
        ...
        <section
          ...
          aria-labelledby="ai-at-work-hero-title"
        >
          <div className="relative">
            ...
            <h1
              id="ai-at-work-hero-title"
              className="mt-5 text-3xl font-semibold tracking-tight text-[color:var(--jf-text)] sm:text-[2.125rem] sm:leading-tight"
            >
              {course.title}
            </h1>
            <p className={`mt-4 max-w-2xl ${bodyProse}`}>{AI_AT_WORK_MICROLEARNING_HERO_DESCRIPTION}</p>
            <p className="mt-4 text-[13px] font-medium text-[color:var(--jf-text)] sm:text-sm">{AI_AT_WORK_MICROLEARNING_METADATA_ROW}</p>
        ...
        <div className="mt-8 overflow-hidden rounded-3xl border border-[color:var(--jf-border)] bg-[color:var(--jf-surface)] shadow-[var(--jf-shadow-soft)]">
          <section className="border-b border-[color:var(--jf-border)] px-6 py-8 sm:px-10 sm:py-10" aria-labelledby="ai-at-work-outcomes-heading">
            <h2 id="ai-at-work-outcomes-heading" className="text-lg font-semibold tracking-tight text-[color:var(--jf-text)] sm:text-xl">
              What you will be able to do
            </h2>
            <ul className="mt-5 space-y-3">
              {AI_AT_WORK_MICROLEARNING_OUTCOMES.map((line) => (
          ...
          <section className="border-b border-[color:var(--jf-border)] px-6 py-8 sm:px-10 sm:py-10" aria-labelledby="ai-at-work-flow-heading">
            <h2 id="ai-at-work-flow-heading" className="text-lg font-semibold tracking-tight text-[color:var(--jf-text)] sm:text-xl">
              Lesson flow
            </h2>
            <ol className="mt-6 space-y-2.5">
              {AI_AT_WORK_MICROLEARNING_LESSON_FLOW.map((title, i) => (
                <li key={title}>
          ...
          <section
            ...
            aria-labelledby="ai-at-work-player-heading"
          >
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <h2 id="ai-at-work-player-heading" className="text-lg font-semibold tracking-tight text-[color:var(--jf-text)] sm:text-xl">
                  Start learning
                </h2>
            ...
            <div className="mt-8 border-t border-stone-200/80 pt-6">
              ...
              {complete ? (
                <p className="mt-3 text-[14px] font-medium text-emerald-800" data-testid="free-starter-ai-at-work-complete-label">
                  {FREE_STARTER_COMPLETION_THANKS}
                </p>
              ) : (
                <button
