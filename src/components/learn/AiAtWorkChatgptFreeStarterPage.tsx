...
const flowSteps: readonly FreeStarterFlowStep[] = AI_AT_WORK_MICROLEARNING_LESSON_FLOW.map((label, i) => ({
  id: `flow-${i + 1}`,
  label,
}))
...
export function AiAtWorkChatgptFreeStarterPage() {
  ...
  return (
    <FreeStarterMicrocourseTemplate
      pageTestId="free-starter-ai-at-work-chatgpt-page"
      course={course}
      heroDescription={AI_AT_WORK_MICROLEARNING_HERO_DESCRIPTION}
      metadataRow={AI_AT_WORK_MICROLEARNING_METADATA_ROW}
      practiceBullets={AI_AT_WORK_MICROLEARNING_OUTCOMES}
      flowHeading="Course outline"
      flowSteps={flowSteps}
      onFlowStepOpen={onFlowStepOpen}
      resume={resume}
      embedHeading="Begin the course"
      iframeTitle={`${course.shortTitle} — interactive lesson`}
      newWindowTestId="free-starter-ai-at-work-open-tab"
      complete={complete}
      onMarkComplete={onMarkComplete}
      primaryCtaTestId="free-starter-ai-at-work-start"
    />
  )
}
