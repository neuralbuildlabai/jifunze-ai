...
import { BusinessAnalyticsModuleTrainingBlock } from './BusinessAnalyticsTrainingMedia'
...
type BusinessAnalyticsStandaloneModulePageProps = {
...
export function BusinessAnalyticsStandaloneModulePage({
  entry,
  module,
  source,
  slides,
...
}: BusinessAnalyticsStandaloneModulePageProps) {
  ...
  const slidesReady = businessAnalyticsDecisionMakingSlideManifest.assetStatus === 'ready' && slides.length > 0
  ...
  if (firstIncomplete) primaryLabel = anyLessonStarted ? 'Continue training' : 'Start module'
      ...
      <div className="mx-auto w-full max-w-4xl space-y-8">
        ...
        <section className="space-y-3">
          ...
          <p className="text-[13px] text-stone-600">
            {module.durationMinutes} min · {slidesReady ? 'slides in player' : 'deck + lessons'} · practice lab
          </p>
          <p className="max-w-2xl text-[15px] leading-relaxed text-stone-700">{purposeLine}</p>
        </section>

        <BusinessAnalyticsModuleTrainingBlock moduleSlug={module.slug} slides={slides} />
        ...
        <div>
          <Link
            ...
            data-testid={`standalone-ba-module-primary-cta-${module.slug}`}
          >
            {primaryLabel}
          </Link>
        </div>
        ...
        <section id="ba-module-completion" className="rounded-xl border border-stone-200/90 bg-white p-5 sm:p-6">
          ...
          <ul className="mt-3 space-y-2 text-[13px] leading-relaxed text-stone-700">
            <li>{slidesReady ? '· Work through the slides above.' : '· Use the downloadable deck and lessons for this module.'}</li>
            <li>· Mark lessons as studied when you finish each checkpoint.</li>
            <li>· Finish the practice lab prompts below.</li>
            {module.moduleQuiz.length === 0 ? (
              <li>· The graded quiz (12 questions) is in Module 6.</li>
            ) : (
              <li>· Pass the Module 6 quiz (12 questions, 75%+) for certificate progress.</li>
            )}
          </ul>
          ...
          {module.moduleQuiz.length > 0 ? (
            <div className="mt-5 rounded-lg border border-stone-200/80 bg-stone-50/50 p-4">
              ...
              <div className="mt-3">
                ...
                >
                  {savedQuizPassed ? 'Retake quiz' : savedQuizScore ? 'Retry quiz' : 'Take Module 6 quiz'}
                </Link>
              </div>
