...
/* -------------------------------------------------------------------------- */
/* Featured course cards (titles + module counts derived from real catalog)   */
/* -------------------------------------------------------------------------- */
...
const SCHOOL_ORDER: FlagshipSchoolId[] = [
  'ai_digital',
  'business_growth',
  'career_intellect',
  'leadership_learning',
...
export type DiscoveryHeroProps = {
  /** Test id for the primary "Explore Courses" CTA. */
  primaryCtaTestId?: string
  /**
   * Where the primary CTA goes. Defaults to anchor-jumping to the in-page
   * #featured-courses section (suitable for /learn). The homepage passes
  ...
  headingId?: string
  /** Optional slot rendered below the CTA row (for trust strips, etc.). */
  belowCtaSlot?: ReactNode
}
...
export function DiscoveryHero({
  ...
  const isAnchor = primaryCtaTo.startsWith('#')
  const isInternalRoute = primaryCtaTo.startsWith('/')
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(251,146,60,0.18),transparent_55%)]"
        aria-hidden
      />
      <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:items-center lg:gap-16 lg:px-8 lg:py-20">
        <div className="max-w-xl">
          ...
          <h1 id={headingId} className="mt-6 text-4xl font-bold tracking-tight text-zinc-900 sm:text-5xl sm:leading-[1.08]">
          ...
          <p className="mt-6 text-[17px] leading-relaxed text-zinc-600 sm:text-lg">
            Build practical AI, data, business, and digital skills through guided courses designed for self-paced learning, revision,
            and real-world outcomes.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            {isAnchor ? (
              <a href={primaryCtaTo} data-testid={primaryCtaTestId} className={primaryCtaClass}>
                Explore Courses
                <IconArrowRight className="h-4 w-4" />
              </a>
            ) : isInternalRoute ? (
              <Link to={primaryCtaTo} data-testid={primaryCtaTestId} className={primaryCtaClass}>
                Explore Courses
                <IconArrowRight className="h-4 w-4" />
              </Link>
            ) : (
          ...
          {belowCtaSlot ? <div className="mt-6">{belowCtaSlot}</div> : null}
...
export function FeaturedCoursesSection({
  sectionTestId,
  cardTestIdPrefix = 'discovery-featured',
}: {
        ...
        <div className="mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURED_COURSES.map((course) => (
            <article
              ...
              data-testid={`${cardTestIdPrefix}-${course.slug}`}
            >
              <Link to={`/learn/courses/${course.slug}`} className="relative block aspect-[16/10] overflow-hidden">
                ...
                <div
                  className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cpath d=\'M0 60L60 0H0z\' fill=\'%23ffffff\' fill-opacity=\'0.06\'/%3E%3C/svg%3E')] opacity-40 mix-blend-overlay"
                  aria-hidden
                />
                <span
              ...
              <div className="flex flex-1 flex-col px-5 pb-5 pt-4">
                ...
                <div className="mt-auto flex items-center justify-between gap-3">
                  <span className="inline-flex items-center gap-1 text-[13px] font-semibold text-zinc-800">
                    ...
                    {course.rating} <span className="font-normal text-zinc-500">({course.learners})</span>
                  </span>
...
export function SchoolsSection({ cardTestIdPrefix = 'discovery-school-card' }: { cardTestIdPrefix?: string }) {
  return (
    <section
      id="schools"
      className="scroll-mt-24 border-y border-zinc-100 bg-zinc-50/50 px-4 py-16 sm:px-6 lg:px-8 lg:py-20"
          ...
          {SCHOOL_ORDER.map((id) => {
            ...
            return (
              <Link
                key={id}
                to={`/learn/school/${id}`}
                data-testid={`${cardTestIdPrefix}-${id}`}
