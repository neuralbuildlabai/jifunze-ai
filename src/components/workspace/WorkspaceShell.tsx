    ...
    <WorkspaceGeneratorProvider>
      ...
      >
        <div className={isLearner ? learnerShellTokens.workspaceInner : 'relative mx-auto max-w-5xl px-4 pb-16 pt-8 sm:pb-20 sm:pt-10'}>
          {isLearner ? (
            <header
              className={`mb-10 flex flex-col gap-6 pb-6 sm:flex-row sm:items-start sm:justify-between ${learnerShellTokens.workspaceHeaderBorder}`}
            >
              <div className="min-w-0">
                <p className={learnerShellTokens.mutedEyebrow}>Jifunze</p>
                <p className="mt-1 text-sm text-zinc-400">Learning workspace</p>
              </div>
              <div className="flex flex-col items-stretch gap-4 sm:items-end">
                {isSupabaseConfigured() && user ? (
                  <button
                    ...
                    className="inline-flex min-h-[2.25rem] shrink-0 items-center justify-center self-end rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 text-xs font-semibold text-zinc-200 transition hover:border-white/[0.12] hover:bg-white/[0.05] disabled:cursor-not-allowed disabled:opacity-50"
                    data-testid="workspace-shell-sign-out"
                  >
                    {signOutPending ? 'Signing out…' : 'Sign out'}
                  </button>
                ) : null}
                <LearnerTopNav className="justify-end" />
              </div>
            </header>
          ) : (
