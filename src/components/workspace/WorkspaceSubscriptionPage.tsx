    ...
    <WorkspaceRouteReady>
      ...
      >
        ...
        <section className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4 ring-1 ring-white/[0.04]">
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-zinc-500">Your account</p>
          <p className="mt-2 text-sm text-zinc-200">
            Signed in as <span className="font-medium text-white">{user?.email ?? '—'}</span>
          </p>
          <p className="mt-2 text-sm text-zinc-400">
            Plan label: <span className="font-semibold text-zinc-100">{tierLabel}</span>
          </p>
          {planHint ? (
            <p className="mt-2 text-[12px] text-zinc-500">
              Typical paid entry: <span className="text-zinc-300">{planHint.name}</span> — actual access follows what you purchase at checkout.
            </p>
          ) : null}
        </section>
        ...
        {liveBillingConfigured ? (
          <section className="rounded-xl border border-white/[0.08] bg-zinc-950/45 p-4 text-[12px] text-zinc-300 ring-1 ring-white/[0.04]">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-zinc-500">Manage payment method</p>
            <p className="mt-2 text-zinc-400">Update cards, invoices, and cancellation through our secure billing portal.</p>
            <button
              type="button"
        ...
        <section className="rounded-xl border border-white/[0.08] bg-black/25 p-4 ring-1 ring-white/[0.04]">
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-zinc-500">Refund help</p>
          <p className="mt-2 text-[12px] leading-relaxed text-zinc-400">
            ...
            . Contact{' '}
            <a className="text-violet-300 hover:text-violet-200" href={`mailto:${SUPPORT_CONTACT_EMAIL}`}>
        ...
        <section className="rounded-xl border border-amber-500/25 bg-amber-950/20 p-4 text-[11px] leading-relaxed text-amber-100/95 ring-1 ring-amber-500/15">
          ...
          <p className="mt-2 text-amber-100/90">
            For invoices, taxes, or payment issues, email{' '}
            <span className="font-medium text-amber-50">{SUPPORT_CONTACT_EMAIL}</span>.
          </p>
        </section>

        <nav className="flex flex-wrap gap-x-4 gap-y-2 border-t border-white/[0.06] pt-6 text-[12px] text-zinc-500">
