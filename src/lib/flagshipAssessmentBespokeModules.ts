...
const FLAGSHIP_ASSESSMENT_BESPOKE_MID_TRIPLES: Partial<Record<string, Trio>> = {
  "ae-m02": [
    mcq(
      'A teammate forwards an AI-generated memo with a confident citation that you cannot find in the journal’s archive. They argue the model "wouldn’t make it up because it sounded technical." Which response is most defensible before the memo is reused with stakeholders?',
      'Treat the unverifiable citation as a load-bearing failure: open the source or downgrade every claim that depends on it to "uncited—needs source," explain the mechanism (citation-shaped strings can appear without a real library behind them), and only reinstate the line when a primary source supports it.',
      'Run the prompt twice more to see if the same citation re-appears; if it does, treat the citation as confirmed because consistent fluent answers are evidence.',
      'Three identical fluent answers can share one weak prior; "sounded technical" is a tone judgment, not a verification step. Bias and fabrication leave the model’s account fluent and your account wrong; the inspection has to happen on the source side, not the model side.',
      'Strip the citation entirely and rewrite the memo without telling the colleague—the missing source is a personal review issue.',
    ),
    scen(
      'Pick the most defensible move under deadline.',
      'Your team’s comms lead pastes AI talking points into a draft: "Recent industry surveys show clear consensus that generative AI cuts knowledge-worker time by 30%." The release goes out tomorrow. The original prompt named no source; the model added an unverifiable "industry survey" string. Two of you remember reading conflicting numbers in independent audits last quarter.',
      'Pause the line, label the 30% figure "uncited—conflicting evidence in audits we can name," surface the disagreement on the page itself, and rewrite to a defensible claim ("some studies report time savings in narrow tasks; others find smaller or no effect in our deployment shape") before send.',
      'Keep the headline because rewriting weakens the announcement and "30%" already appears in earlier marketing copy.',
      'Polished averages flatten disagreement and become future corrections under media scrutiny. The mature move is to make the conflict visible and let the recommendation stand or fall on what the team can actually defend—not on the most quotable sentence.',
      'Refuse to release any version of the announcement until every figure is independently re-audited from scratch.',
    ),
    refl(
      'Applied evidence · myth-correction sheet + load-bearing inspection',
      'For one belief I or my team held about AI ("the model knows our customers," "longer prompts mean better answers," "fluency means it checked its sources"), I wrote a one-page myth correction with mechanism + boundary + verification step, and I ran a load-bearing inspection on a real AI output: I underlined the highest-stakes sentence, classified each claim as verified / inferred / uncited, and named the cheapest next observation that would change the outcome before I trusted the draft.',
    ),
  ],
  "ae-m03": [
    mcq(
      'You are rewriting a prompt you reuse weekly so a colleague could run it tomorrow without DMing you for context. Which addition gives you the most control without bloating the prompt?',
      'Add audience plus one forbidden move plus an output shape: who the answer is for, what the model must not invent ("do not legalize policy text; if a date is missing, return TBD"), and the structure a reviewer will scan (table columns or labeled sections)—so omission and invention show up where a peer can see them.',
      'Add three more adjectives ("be thoughtful, rigorous, and authoritative") to lift the tone and let the model infer the rest from a confident voice.',
      'Tone words let the model guess everything that matters—audience, refusal behavior, format. Naming audience, one forbidden move, and an output shape converts wishes into a contract the model can fail visibly instead of failing politely.',
      'Paste the entire policy document or transcript into the prompt without trimming so the model "has all the context."',
    ),
    scen(
      'Pick the most defensible operational move.',
      'A small CS team wants to draft refund replies with AI. The first prompt is "Write a polite refund email for this customer." The model invents promised re-issue dates and adds an apology paragraph that contradicts the current product policy. Real customers receive these messages.',
      'Rewrite the prompt as a small contract: role (CS rep, non-lawyer), audience (the named customer with delay context), inputs (the approved policy snippet only), forbidden moves (no invented timelines, no legal conclusions, no overpromise; if a date is missing, return [DATE TBD]), output shape (subject + 3-paragraph body + escalation flag), and one acceptance test (the email cannot mention any timeline absent from the policy snippet).',
      'Tell agents to "edit harder" before sending and rerun the same vague prompt across the queue.',
      'Editing harder treats prompt drift as a willpower problem; in production, willpower fails on a Friday afternoon. A prompt contract puts the rules in a place the model cannot quietly skip and a reviewer can audit by glancing at the schema, not by re-reading every reply.',
      'Ban AI for refund replies entirely and route everything to the most senior agent with no template at all.',
    ),
    refl(
      'Applied evidence · prompt contract + acceptance test',
      'For one workflow I actually run, I authored a prompt contract with task / context / audience / constraints / output shape / two forbidden moves / one mechanical acceptance test ("if claim X has no source in the inputs, return TBD"), ran the contract on a real input, recorded which failure mode the contract caught that the old prompt was hiding, and named the data tier this prompt may and may not see.',
    ),
  ],
  "ae-m05": [
    mcq(
      'Which iteration habit makes a prompt improvement defensible to a reviewer who was not in the room when you ran the test?',
      'Save a fixture (frozen inputs + a small rubric), change one variable per version, log the diff with a hypothesis and the observed rubric movement, and roll back when a "better" variant trades away a non-negotiable row (policy compliance, citation coverage, refusal behavior, format)—not when the tone simply feels worse.',
      'Retry with new wording until the answer "feels right" and ship the variant the most senior person in the room prefers when the meeting ends.',
      'Tone is not a test. Without fixtures and a rubric, every "better" output is an anecdote and every regression hides until a customer or auditor finds it. Logged diffs and named regression rows convert iteration from vibes into something a peer can ship from on Monday.',
      'Change three things at once because that is faster than running disciplined A/B comparisons under deadline.',
    ),
    scen(
      'Pick the strongest iteration response.',
      'You upgraded a customer-reply prompt from v1 to v3 last week and CSAT moved up. Today, finance flags increased goodwill credits tied to language v3 introduced ("we will refund the difference promptly") that the policy does not authorise. Your fixture set never included a "creates new obligations" rubric row.',
      'Roll back to v1 immediately for any reply that touches money; add "creates new obligations" as a non-negotiable rubric row; rebuild a golden fixture set that includes adversarial tickets where v3 was tempted to overpromise; only re-test v3 against the new row and an explicit policy-snippet anchor before any further rollout.',
      'Keep v3 running because CSAT is up and write a memo telling agents to "edit out the refund line manually before sending."',
      'Manual edits decay under load. A regression row that costs real money is a contract violation, not a phrasing preference; the discipline is rollback first, then re-test with the row that should have caught it. CSAT cannot pay for unauthorised obligations on its own.',
      'Delete the prompt history so the regression is not visible in the next quarterly review.',
    ),
    refl(
      'Applied evidence · fixture + diff log + rollback rule',
      'For one prompt I iterated, I saved a fixture (frozen inputs + 4–6 rubric rows including at least one non-negotiable), ran v1 vs v2 on the same fixture, recorded the rubric movement and one tradeoff in plain language, and wrote a rollback rule naming the specific row whose regression would force me to revert—even if other rows improve.',
    ),
  ],
  "sw-m02": [
    mcq(
  ...
  "ae-m07": [
    mcq(
      'You are tailoring an AI-assisted message for two readers (a regulator and a customer) and the model offers a "warmer" rewrite that lands beautifully. What is the most professional move before send?',
      'Separate the lanes: keep one approved facts block as the source of truth; let tone vary across audiences but forbid the model from promoting any sentence to a fact unless it appears in the facts block; tag inferred claims and unknowns explicitly; require a named human releaser per audience before send.',
      'Adopt the warmer rewrite because tone approval from a senior colleague signals the message is factually ready as well.',
      'Tone approval is not factual approval; warmth often smuggles new commitments ("we will fix this by Friday") that policy does not back. Locked facts plus separated tone lanes plus a named releaser is how teams keep audience fit honest under deadline pressure.',
      'Send identical text to both audiences to "stay consistent" even though the regulator needs precision the customer does not.',
    ),
    scen(
      'Pick the most defensible move before send.',
      'A non-profit fundraising lead asks the model to make a donor email "more emotional." The rewrite adds a child’s age and a quote from a beneficiary that read beautifully and were not in the approved case file. The campaign launches in two hours; the lead is tired and ready to ship.',
      'Refuse the unsourced details; reinstate only beneficiary language explicitly approved in the case file; add a tag system in the draft (APPROVED_FACT / ORG_VOICE / NEEDS_HUMAN_FACT) that the model is forbidden to upgrade; require a second human pass on factual claims before send; record the gap so the next iteration of the prompt blocks the invention pattern by default.',
      'Send the warmer version because the made-up details "sound like things real beneficiaries say" and the deadline is real.',
      'Inventing affecting detail in fundraising is a future correction—at minimum—and a trust failure for the people the work claims to represent. The discipline is small (locked facts, named releaser, tag system) and the alternative cost is much larger than two extra hours of editorial review.',
      'Cancel the campaign entirely because any AI-touched message risks fabrication and that risk cannot be reduced.',
    ),
    refl(
      'Applied evidence · audience-fit memo + verification footer',
      'For one real piece of writing I owe (memo, donor note, customer reply, or internal brief), I drafted two openings tuned to two audiences using only an approved facts block, tagged each sentence as APPROVED_FACT / ORG_VOICE / NEEDS_HUMAN_FACT, added a verification footer naming the source for any load-bearing claim, and named the human releaser who must sign before any version goes out.',
    ),
  ],
  "ae-m09": [
    mcq(
      'A new hire asks "what is allowed and what is not when I use AI here?" Which guardrail design is operational rather than aspirational?',
      'A short, role-specific note that names assist-only zones, forbidden zones (with one concrete example each), the disclosure pattern that tells reviewers what to re-check (not just "AI-assisted"), the named human owner per AI-assisted artifact, and the single escalation contact for suspected data mishandling—signed by a named person on a dated page.',
      'A long ethics deck that asks employees to "act with integrity, transparency, and care" and reminds them AI is a tool that requires good judgment.',
      'Slogans collapse under deadline pressure. Operational guardrails name behaviours, owners, and escalation paths a tired teammate can follow at 11 p.m. Aspirational language without owners diffuses accountability and incentivises hiding when something goes wrong.',
      'A blanket ban on AI use across all roles, enforced by an honor system with no review process or escalation path.',
    ),
    scen(
      'Pick the most defensible move.',
      'A hiring manager has been using a public AI chat to summarise candidate CVs and draft rejection emails. Some rejections are unusually specific in ways that imply review the manager did not perform; CV PII has flowed into a tool with no enterprise agreement. A peer raises the issue privately the day before a hiring committee.',
      'Stop further candidate PII paste; preserve logs and copies; notify the security or compliance lead per the incident playbook; route hiring back to the approved tooling with a rubric-aligned extraction pattern; require second-human review on any rejection language for fairness and accuracy; document the gap in the team’s responsible-use note so the next manager inherits the rule, not the mistake.',
      'Have the manager delete the chat history and the rejection drafts and continue because no candidate has complained yet.',
      'Containment plus controlled escalation keeps the issue solvable; "delete and hope" destroys the evidence you would need if a candidate, regulator, or auditor asks. Punishing disclosure trains people to hide; the durable fix is a written rule that survives the next manager rotation.',
      'Automate rejection emails with the same chat going forward so at least the language is consistent across candidates.',
    ),
    refl(
      'Applied evidence · responsible-use note + disclosure pattern',
      'For one role or team I touch, I drafted a one-page responsible-use note with assist-only zones, two concrete forbidden examples, a disclosure line that tells reviewers what to re-check (not just "AI-assisted"), the named human owner per artifact, and the escalation contact for suspected data mishandling—and I named one workflow I will not automate this quarter, with the data class and the risk-to-people that justify the refusal.',
    ),
  ],
  "ae-m10": [
    mcq(
  ...
  "ae-m11": [
    mcq(
      'Which extraction prompt protects research synthesis from quietly laundering disagreement when the model writes the brief?',
      'Require atomic claims with paragraph or cell anchors, a strength label per claim (Strong / Moderate / Weak / Uncited), an explicit conflicts row when sources disagree, and a "what would change my conclusion" line; forbid invented citations and require TBD where the source is missing.',
      'Ask for "a confident executive summary in five sentences" and trust that the model will surface the conflicts if they actually matter.',
      'Executive concision is not the same as evidence honesty. Five-sentence summaries reward smoothing; explicit anchors, strength labels, and a falsifier line force the model to put claims somewhere a reviewer can attack them by name.',
      'Ask the model which sources are most trustworthy and remove the others before extraction so the brief stays clean.',
    ),
    scen(
      'Pick the most defensible move under deadline.',
      'You are briefing a division head by 5 p.m. The AI synthesis names two reports that disagree on uplift (one vendor case study, one independent audit), rounds the disagreement into a single percentage, and adds a benchmark sentence with no citation. The exec wants a clear recommendation tonight.',
      'Refuse the false-precision number; rewrite the brief with the conflict on page one (vendor study vs independent audit, with anchors); strip the uncited benchmark sentence; recommend a staged commitment with a named falsifier ("if the next two cycles miss X, the recommendation flips to pause-and-audit") and the cheapest next information buy that would adjudicate; state the unknown the brief still owes.',
      'Average the two percentages and pick the midpoint so leadership receives a single confident number tonight.',
      'Averaging conflicts hides where the team is gambling and turns the brief into a future correction. Reviewers prefer substance they can challenge—visible disagreement, a falsifier, a staged commitment—to an executive-friendly verdict that cannot survive the next data point.',
      'Refuse to brief at all because no single recommendation is yet defensible and any partial answer would mislead.',
    ),
    refl(
      'Applied evidence · evidence table + executive brief with conflict visible',
      'For one real research question, I built an evidence table (claim / source anchor / strength / conflict noted? / next observation) for at least five load-bearing claims, wrote a one-page brief that keeps at least one unresolved conflict visible in the body (not buried in an appendix), named the cheapest next information buy, and stated what observation would flip the recommendation—signed by me, not by the model.',
    ),
  ],
  "ae-m12": [
    mcq(
  ...
  "ae-m15": [
    mcq(
      'A new teammate inherits your prompt pack on a Monday morning. Which property tells you the pack is professional rather than a folder of "final v7 really final" screenshots?',
      'Each entry has purpose, audience, inputs (with data tier), boundaries and refusal rules, output shape, named owner, version notes that say what each version targeted, an acceptance test, and a changelog row that names what triggered the last edit (model update, regression, policy change).',
      'The folder is named "FINAL" and the senior author maintains it personally so newcomers should defer to whatever the most recent screenshot shows.',
      'Personal stewardship and confident folder names disappear when the maintainer goes on leave or the model updates. The pack is professional when a tired teammate can run it cold, see why each version exists, and run an acceptance test that catches the failure mode the changelog already names.',
      'It uses the longest prompts available so the model "has every possible instruction" embedded directly in the pack itself.',
    ),
    scen(
      'Pick the most defensible response to a vendor model update.',
      'A vendor pushes a model update mid-week. Your shared customer-reply pack starts producing slightly different refusals; half the team patches their copy quietly to "make it sound right again." A few replies drift away from the team’s policy snippet without anyone tracking the change.',
      'Freeze edits; diff active prompts against the canonical pack entry; restore the named owner’s version as the source of truth; re-run the pack’s acceptance tests against the updated model and a small adversarial fixture; only reopen edits via versioned changes with a changelog line and an owner—reverting any silent patches that cannot be reproduced from the canonical entry.',
      'Let everyone keep their personal patches because "the team knows what works" and re-converge later when things settle on their own.',
      'Silent patching is how shared systems die. The mature move is to treat the model update as an event that triggers re-test, not a reason to fork. Acceptance tests + a named owner + a changelog is what keeps the pack survivable across vendor changes and team turnover.',
      'Stop using the pack entirely and let each teammate write their own prompts from memory.',
    ),
    refl(
      'Applied evidence · prompt-pack slice with acceptance test + changelog',
      'For one real workflow, I assembled a prompt-pack slice with three entries (purpose / audience / inputs with data tier / boundaries / output shape / named owner / version notes / one acceptance test) and a changelog row that names what triggered the most recent edit, then ran the pack on a fresh scenario, logged at least two gaps the test surfaced, and named the rule that would freeze edits until the owner reviews.',
    ),
  ],
  "ae-m16": [
    mcq(
