-- Single RPC shape: bootstrap_my_workspace(workspace_name text).
-- Drops the legacy zero-argument overload so PostgREST cannot pick between candidates.

drop function if exists public.bootstrap_my_workspace();
...
begin
  ...
  insert into public.tenants (name) values (new_name) returning id into tid;
...
comment on function public.bootstrap_my_workspace(text) is
  'Ensures auth.uid() has a tenant: returns existing default_tenant_id, repairs profile from membership, or creates workspace + profile row. Optional workspace_name labels a new tenant (null/blank => default name).';
