-- Richer curriculum fields for training plans, modules, lessons, and quiz questions.
...
comment on column public.training_plans.expected_outcomes is
  'Bullet-style or paragraph outcomes learners should achieve by the end of the plan.';

alter table public.training_modules
  add column if not exists module_goal text;

alter table public.training_modules
  add column if not exists why_it_matters text;

comment on column public.training_modules.module_goal is 'What this module aims to achieve in the larger arc.';
comment on column public.training_modules.why_it_matters is 'Optional motivation/context for the module.';
...
alter table public.training_lessons
  add column if not exists action_exercise text;

alter table public.training_lessons
  add column if not exists reflection_prompt text;

alter table public.training_lessons
  add column if not exists mistakes_to_avoid text;

alter table public.training_lessons
  add column if not exists estimated_minutes int;

comment on column public.training_lessons.lesson_summary is 'Purpose / overview before main content.';
comment on column public.training_lessons.estimated_minutes is 'Optional estimated time to complete.';
...
alter table public.training_quiz_questions
  add column if not exists source_lesson_id uuid references public.training_lessons (id) on delete set null;

alter table public.training_quiz_questions
  drop constraint if exists training_quiz_questions_difficulty_check;

alter table public.training_quiz_questions
  add constraint training_quiz_questions_difficulty_check
  check (difficulty is null or difficulty in ('easy', 'medium', 'hard'));

comment on column public.training_quiz_questions.explanation is 'Shown after submit to reinforce learning.';
comment on column public.training_quiz_questions.difficulty is 'Optional: easy | medium | hard.';
comment on column public.training_quiz_questions.source_lesson_id is 'Optional link to the lesson this question draws from.';

-- ---------------------------------------------------------------------------
-- create_training_plan_from_seed: extended JSON + lesson id map for source_lesson_index
-- ---------------------------------------------------------------------------
...
comment on function public.create_training_plan_from_seed(uuid, jsonb) is
  'Atomically creates training plan with rich optional fields; quiz questions may reference lessons by source_lesson_index.';
